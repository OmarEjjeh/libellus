...
- derive the Draft's shape from the schema instead of hand-writing it. ADR-0049 decision 1 keeps the form's `leererZustand` and its two translators by hand for one round; once #92 makes the schema Zod, a form library (react-hook-form + `zodResolver`, or TanStack Form) can generate the form shape, its defaults and its coercion from `z.input`/`z.output` rather than restating them. Wanted, not urgent — the translators work and are tested
- revisit `@vagdur/exsurge` (1.28.0, published 2026-08-01) instead of the vendored jgabc blob. ADR-0049 decision 6 vendors the pinned copy because npm's own `exsurge` is a 2016 stub at `0.0.0` and the fork was days old at the time. If it is still maintained in a year it is a real package with real versioning, and the vendored blob could retire
- the editor's problems panel wants a second Pyodide worker. ADR-0049 decision 9 defers debounced background validation because one worker serves requests serially and a build holds it for minutes. Moot if #92 lands first, since validation stops needing the worker at all — so this is only worth building if the TypeScript port stalls
